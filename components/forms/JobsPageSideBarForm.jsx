...
function JobsPageSideBarForm({
  ...
  const jobTypesOptions = [
    ...
    { value: "Intership", display: "Intership" },
    { value: "Contract", display: "Contract" },
  ];

  const experienceLevelsOptions = [
    { value: "Junior", display: "Junior" },
    { value: "Mid", display: "Mid" },
    { value: "Senior", display: "Senior" },
    { value: "Tech-lead", display: "Tech Lead" },
  ];

  const baseSalaryRangesOptions = [
    { value: "<10K", display: "< €10K", bounds: { min: 0, max: 10000 } },
    {
      value: "10K-20K",
      display: "€10K - €20K",
      bounds: { min: 10001, max: 20000 },
    },
    {
      value: "20K-30K",
      display: "€20K - €30K",
      bounds: { min: 20001, max: 30000 },
    },
    {
      value: "> 30K",
      display: "> €30K",
      bounds: { min: 30001, max: 1000000 },
    },
  ];
  ...
  const handleExperienceLevelsSelect = (e, option) => {
    ...
    if (e.target.checked) {
      setSideBarFormState((prevState) => {
        const experienceLevel = [...prevState.experienceLevel];
        experienceLevel.push(option);
        return { ...prevState, experienceLevel };
      });
      // setSideBarFormState((prevState) => {
      ...
      setSideBarFormState((prevState) => {
        ...
        return {
          ...prevState,
          experienceLevel: prevState.experienceLevel.filter(
            (experienceLevel) => option != experienceLevel
          ),
        };
      });
                  ...
                  <li key={option.value}>
                    <label className="flex items-center">
                      <input
                        ...
                        //   option.value
                        // )}
                        checked={sideBarFormState.experienceLevel.includes(
                          option.value
                        )}
                      />
                      <span className="text-sm text-slate-600 font-medium ml-2">
